...
const EditTag: React.FC<EditTagProps> = ({
  ...
  const [tagName, setTagName] = useState(currentTagName);
  ...
  const handleEditTag = async () => {
    ...
    if (tagName.trim() === currentTagName.trim()) {
    ...
    try {
      const response = await apiClient.post("/v1/tag/edit_tag_info", requestData);
      ...
      if (response.data.code === 200) {
        ...
        if (response.data.code === 400 && response.data.message.includes("already exists")) {
          setError("A tag with this name already exists. Please choose a different name.");
        } else {
          setError(`Failed to update tag: ${response.data.message}`);
        }
      }
    } catch (error: unknown) {
        ...
        if (error.response?.status === 400 && error.response?.data?.message?.includes("already exists")) {
          ...
          setError(
            error.response?.data?.message ||
              `Error (${error.response?.status}): Failed to update tag.`
          );
        }
      } else {
  ...
  return (
    <Overlay>
      <Modal ref={modalRef}>
        <ModalHeader icon={TagIcon} title="Edit Tag" onClose={onClose} />
        <InputContainer>
        ...
        <ButtonContainer>
          ...
          <Button 
            ...
            disabled={isSubmitting || !tagName.trim() || tagName.trim() === currentTagName.trim()}
          >
