...
- **转折点判断**: 只要方向发生变化（减→增 或 增→减）就保留，**不检查变化量阈值**
...
**条件：**
- `prevChange <= 0` (前一变化为减少或持平)
- `nextChange > 0` (后一变化为增加)
- **不检查变化量阈值** - 只要方向改变就保留

**保留逻辑：**
保留当前版本作为减 → 增的转折点（局部最小值）。
...
结果：版本3 被识别为"减→增"转折点（局部最小值）并保留
```

#### 5.2 增 → 减 转折点（局部最大值）

**条件：**
- `prevChange >= 0` (前一变化为增加或持平)
- `nextChange < 0` (后一变化为减少)
- **不检查变化量阈值** - 只要方向改变就保留

**保留逻辑：**
保留当前版本作为增 → 减的转折点（局部最大值）。
...
之前的逻辑要求 `|prevChange| >= 2` 或 `|nextChange| >= 2`，但这会导致一些重要的转折点被过滤掉。

**问题示例：**
```
"今天去麦理浩径"(7) → "今天去"(3) → "今天去迪"(4)
                    ↓ -4        ↓ +1

如果要求变化量 >= 2，"今天去"(3) 会被过滤（nextChange = +1 < 2）
```

**解决方案：**
只要发生方向变化（减→增 或 增→减），就保留该转折点，不管变化量有多小。这样可以确保所有的局部极值点都被保留。

#### 5.4 禁用内容包含检查

**问题：**
之前启用了 `CONTENT_CONTAIN_CHECK`，会在添加事件到队列时检查内容包含关系并替换：

```
队列: ["今天去麦理浩径", "今天去"]
添加: "今天去迪士尼" (包含 "今天去")
结果: ["今天去麦理浩径", "今天去迪士尼"] ← "今天去" 被替换掉了！
```

**解决方案：**
禁用 `CONTENT_CONTAIN_CHECK`，改为完全依赖转折点过滤：

```typescript
CONTENT_CONTAIN_CHECK: false,  // 禁用内容包含检查
TURNING_POINT_FILTER: true,    // 启用转折点过滤
```

这样所有的事件都会先进入队列，然后由转折点过滤统一处理，确保转折点不会被提前替换掉。

## 保留规则

系统在每次添加事件到队列时会自动应用以下过滤规则：

**只保留转折点和当前状态** - 记录用户输入行为的关键变化（减→增、增→减）

### 实现细节

- **过滤位置：** `tracker.ts` 的 `queueEvent()` 函数中，每次添加事件后立即过滤
- **过滤函数：** `applyTurningPointFilterInQueue()`
- **配置开关：** `FEATURES.TURNING_POINT_FILTER`（默认 `true`）
- **阈值设置：** 无阈值限制，只要方向改变就保留

### 完整示例

假设用户输入过程：`"今天去麦理浩径" → 删除到 "今天去" → 改为 "今天去迪士尼"`
...
步骤2: 用户删除到 "今天去麦"
  记录2: 长度 4, 内容 "今天去麦" (减少 3)
  队列: [记录1(起点), 记录2(当前)]

步骤3: 用户继续删除到 "今天去"
  记录3: 长度 3, 内容 "今天去" (减少 1)
  队列: [记录1(起点), 记录3(当前)]
  分析: 记录2被过滤（连续减少，不是转折点）

步骤4: 用户输入 "今天去迪士尼"
  记录4: 长度 6, 内容 "今天去迪士尼" (增加 3)
  队列转折点分析:
    - 记录1(7): 起点 ✅ 保留
    - 记录3(3): 7→3→6，前面减少(-4)，后面增加(+3) ✅ 保留（减→增转折点）
    - 记录4(6): 当前状态 ✅ 保留
...
```
记录1: "今天去麦理浩径" (长度 7) ← 增→减转折点（局部最大值）
  ↓ 减少 4
记录3: "今天去" (长度 3) ← 减→增转折点（局部最小值）
  ↓ 增加 3
记录4: "今天去迪士尼" (长度 6) ← 当前状态
```


### 示例2：连续增加后减少

假设用户输入：`"今天去" → "今天去麦理浩径" → "今天去麦理"`

```
记录1: "今天去" (长度 3)
  ↓ 增加 4
记录2: "今天去麦理浩径" (长度 7) ← 局部最大值（增→减转折点）
  ↓ 减少 1
记录3: "今天去麦理" (长度 6) ← 当前状态
```
...
```
✅ 记录2: 长度 7  "今天去麦理浩径" (增→减转折点/局部最大值)
✅ 记录3: 长度 6  "今天去麦理" (当前状态)
```

**说明：** "今天去麦理浩径" 作为局部最大值被正确保留。

### 示例3：包含空内容的序列

假设用户输入过程：`空 → "abc" → "ab" → "a" → "ab"`

```
记录1: 长度 0 (空内容)
  ↓ 增加 3
记录2: 长度 3 "abc" ← 局部最大值（增→减转折点）
  ↓ 减少 1
记录3: 长度 2 "ab"
  ↓ 减少 1
记录4: 长度 1 "a" ← 局部最小值（减→增转折点）
  ↓ 增加 1
记录5: 长度 2 "ab" ← 当前状态
```
...
```
✅ 记录2: 长度 3  "abc" (增→减转折点/局部最大值)
✅ 记录4: 长度 1  "a" (减→增转折点/局部最小值)
✅ 记录5: 长度 2  "ab" (当前状态)
```
...
```
✅ 记录1: 长度 7  "今天去麦理浩径" (增→减转折点/局部最大值)
✅ 记录3: 长度 3  "今天去" (减→增转折点/局部最小值)
✅ 记录4: 长度 6  "今天去迪士尼" (当前状态)
```

**结果：** 只保留转折点和当前状态，完整反映了用户的输入意图变化轨迹。
